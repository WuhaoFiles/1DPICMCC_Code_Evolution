Module DiagnosticsMomentum
    ...
    Type ParticleMomentumOne!(Nx)
                        ...
                        !Real(8) :: QdM
                        Real(8) ::  RhoOne(1:NxMax),ChiOne(1:NxMax),FieldEnergy(2)
                        Real(8) ::  JxOne(1:NxMax),JyOne(1:NxMax),JzOne(1:NxMax)
                        Real(8) ::  TOne(1:NxMax)
								!Real(8),save :: Energy(2) 
    EndType ParticleMomentumOne
    contains

     Subroutine  DiagParticleFieldPeriod(GD,G0D,CF,PB,FG,Mode)
         Implicit none
         ...
         Integer(4) :: i,j,Shift,Shift2,NSpecy
         Real(8) :: HeatingRate(FG%Nx),HeatingPower(2),ParticleAddPower(2),CollitionPowerLoss(2),FieldPower
         Select Type (GD)
             Type is (Grid1D(*,*))
                 Select Case (Mode)
                    case(0)
                        Shift=1
								...
								HeatingPower  = 0.d0
								FieldPower    = 0.d0
								ParticleAddPower = 0.d0
								CollitionPowerLoss = 0.d0
                        Do i=1,CF%Ns+1
                              ...
                              Call GD%Update(GD%Nx,TempPMO%JxOne,Shift)
										...
										Call GD%Update(GD%Nx,HeatingRate,Shift)
									   	HeatingPower(i)       = sum(HeatingRate(:)) * CF%ElectrodeArea*CF%Dx
										   ParticleAddPower(i)   = (PB(i)%MccEnergy(1)-PB(i)%Energy(2))*CF%ElectrodeArea/CF%Dt
										   CollitionPowerLoss(i) = (PB(i)%MccEnergy(1)-PB(i)%Energy(1))*CF%ElectrodeArea/CF%Dt
										Call G0D%Update(PB(i)%MccEnergy(1)*CF%ElectrodeArea,Shift2)
										Call G0D%Update(PB(i)%Energy(1)*CF%ElectrodeArea,Shift2)
										Call G0D%Update(PB(i)%Energy(2)*CF%ElectrodeArea,Shift2)
										Call G0D%Update(HeatingPower(i),Shift2)
					               Call G0D%Update(ParticleAddPower(i),Shift2)
										Call G0D%Update(CollitionPowerLoss(i),Shift2)
								End do
								Call G0D%Update(FG%FieldEnergy(1),Shift2)
								Call G0D%Update(FG%FieldPower,Shift2)
								Call G0D%Update(sum(HeatingPower),Shift2)
								Call G0D%Update(sum(ParticleAddPower),Shift2)
								Call G0D%Update(sum(CollitionPowerLoss),Shift2)
  ...
  subroutine WeightingParticleMomentum(PB,PMO)
                ...
                do i=1,PB%Npar
                   ...
                   PB%Energy(1) = PB%Energy(1) + Energy !!!!!!!
						 Energy = Energy/JtoeV
   ...
   subroutine FieldPowerCaculate(FG,CF)
	   implicit none
		Class(ControlFlow), intent(in) :: CF
		Type(Field),intent(inOUT) :: FG
		FG%FieldEnergy(2) = FG%FieldEnergy(1) 
		FG%SUBFieldEnergy(1) = sum( FG%Ex(1:INT(FG%Nx/2))*(1 + FG%Chi(1:INT(FG%Nx/2)) )*FG%Ex(1:INT(FG%Nx/2)) ) - 0.5*FG%Ex(1)*(1+FG%Chi(1))
		FG%SUBFieldEnergy(2) = sum( FG%Ex(int(FG%Nx/2+1):FG%Nx)*(1+FG%Chi(int(FG%Nx/2+1):FG%Nx))*FG%Ex(int(FG%Nx/2+1):FG%Nx))-0.5*FG%Ex(FG%Nx)*(1+FG%Chi(FG%Nx))*FG%Ex(FG%Nx)
		FG%SUBFieldEnergy = 0.5*FG%SUBFieldEnergy*Epsilon*CF%ElectrodeArea*CF%Dx
		FG%FieldEnergy(1) = sum(FG%SUBFieldEnergy)
		if(CF%Timer == 0) then
			FG%FieldPower = 0.d0
		else
			FG%FieldPower = (FG%FieldEnergy(1)-FG%FieldEnergy(2))/CF%Dt
		end if	
		return
	end subroutine FieldPowerCaculate
