MOdule ModuleTrackingParticle
    ...
    type Tracking_Particle_Information
        ...
        integer(4) :: Particle_Number(2) = (/2,1/)
    ...
    contains
         ...
         subroutine Tracking_Particle(PB,TPI,CF,ti,tj,TP_Temp)
             ...
             real(8) :: ValueTemp(20) = 0,xtemp
             integer(4) :: i,j,k=1
				  !TP_Temp%Temp_Information(TP_Temp%shift,1) = Dble(ti + tj * CF%Period) * CF%Dt
              do i=1,size(TPI%Particle_Type)
                 do j=1,size(TPI%Particle_Number)
						   xtemp = PB(TPI%Particle_Type(i) + 1)%PO(TPI%Particle_Number(j))%X * PB(TPI%Particle_Type(i) + 1)%XFactor
                     ValueTemp(k) = PB(TPI%Particle_Type(i) + 1)%PO(TPI%Particle_Number(j))%X * PB(TPI%Particle_Type(i) + 1)%XFactor
                     k = k + 1
							ValueTemp(k) = PB(TPI%Particle_Type(i) + 1)%PO(TPI%Particle_Number(j))%Energy(PB(TPI%Particle_Type(i) + 1)%Mass,PB(TPI%Particle_Type(i) + 1)%VFactor)/JtoeV
                     k = k + 1 
							!if((xtemp>=ZLength).OR.(xtemp<=0.d0)) then
							!   PB(TPI%Particle_Type(i) + 1)%PO(TPI%Particle_Number(j))%X = 0.5*ZLength/PB(TPI%Particle_Type(i) + 1)%XFactor					  
							!end if
                 end do
					end do
               k = 1
               TP_Temp%Temp_Information(TP_Temp%shift,1) = Dble(ti + tj * CF%Period) * CF%Dt
               do j=2,(2*size(TPI%Particle_Type) * size(TPI%Particle_Number)+1)
         ...
         subroutine  Tracking_Particle_Temp_Initial(CF,TPI,TP_Temp)
             ...
             TP_Temp%Shift = 1
             Array_Column = (2*size(TPI%Particle_Type)*size(TPI%Particle_Number)+1)
