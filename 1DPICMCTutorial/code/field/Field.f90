Module ModuleField
     ...
     Implicit none
                  Type Field !(Nx)
                        ...
                        Real(8) ::  Chi(1:NxMax)
								Real(8) ::  FieldEnergy(2),SUBFieldEnergy(2),FieldPower=0.d0   !Diag
						contains
						      Procedure :: Initial => InitialField
	...
	contains
	subroutine InitialField(FG,CF)
	      Implicit none
			class(Field),intent(inout) :: FG
			CLASS(ControlFlow),INTENT(IN) :: CF
			!ALLOCATE(FG%DiagField_Saving(2,CF%NDiagShort*CF%Period))
	      return
